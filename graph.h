...
private:
  ...
  struct Vertex {
    ...
    explicit Vertex(string uniqueLabel) { this->label = move(uniqueLabel); }
  };

  // graph of vertexs
  vector<Vertex *> vertGraph;

  // helper to find pos of vertex
  int findHelper(const string &label) const;

  // helper to find neighbor of vertex
  int neighborHelper(const string &vert, const string &label) const;

  // helper for prim algorithm
  void primHelper(Vertex *current,
                  map<Vertex *, pair<vector<string>, int>> &edgeMap, int dist,
                  vector<string> path, map<Vertex *, int> &distMap,
                  int covered) const;
